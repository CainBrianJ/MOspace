...
<!--
    Document   : MOspace-local.xsl
    Created on : April 15, 2013 8:30pm
    Author     : pottingerhj (based on the work of OSU:stamper.10)
    Description:
...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template name="buildHeadMOspace">
        ...
        <!-- MOspace - Custom Javascript for entire application. -->
        <script type="text/javascript">
            <xsl:attribute name="src">
                <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]"/>
        ...
        <link rel="shortcut icon " type="image/x-icon">
            <xsl:attribute name="href">
                <xsl:value-of select="/dri:document/dri:meta/dri:pageMeta/dri:metadata[@element='contextPath'][not(@qualifier)]"/>
                <xsl:text>/static/images/favicon.ico</xsl:text>
            </xsl:attribute>
        </link>
...
this template completely replaces the original, found below-->
    ...
    <xsl:template name="ms-simple-search">
